...
import { ExtractionResult, ExtractedMethod, FieldInfo } from './types';
import { CodeOptimizer, OptimizationConfig } from './codeOptimizer';
...
export class OutputGenerator {
  ...
  constructor(config?: OptimizationConfig) {
    ...
    this.optimizer = new CodeOptimizer(this.config);
  }
   ...
   * @returns 输出文件路径
   */
  async generate(result: ExtractionResult, outputDir: string, suffix: string = ''): Promise<string> {
    // 按文件分组
   ...
   * 生成文件头部注释
   */
  private generateHeader(result: ExtractionResult): string {
...
`;
   ...
   * 写入文件
   */
  private async writeToFile(content: string, outputDir: string, entryPoint: string, suffix: string = ''): Promise<string> {
   ...
   * 按文件分组方法
   */
  private groupByFile(methods: ExtractedMethod[]): Map<string, ExtractedMethod[]> {
    const groups = new Map<string, ExtractedMethod[]>();
    for (const method of methods) {
